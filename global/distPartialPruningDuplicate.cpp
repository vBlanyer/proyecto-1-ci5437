...
int main(int argc, char **argv)
{
    ...
    while (!open.Empty())
    {
        ...
        while ((ruleid = next_ruleid(&iter)) >= 0)
        {
            ...
            int new_history = next_bwd_history(history, ruleid); // next history for child
            child_history = next_bwd_history(history, ruleid);
            ...
            if ((old_child_d == NULL) || (*old_child_d > child_d))
            {
                ...
                state_map_add(map, &child, child_history);
